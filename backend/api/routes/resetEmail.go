...
// ResetEmail is responsible for saving new email address for user in database
func ResetEmail(w http.ResponseWriter, r *http.Request, ess *setup.Essentials, secrets *setup.Secrets) {
	...
	// if method is post, save the new email in the database
	if r.Method == "POST" {
		...
		// update email for the user in the database
		sql := `UPDATE app.users SET email = $1 WHERE username = $2;`
		_, err = ess.PG.Exec(sql, dataStruct.Email, usernameCookie.Value)
		if err != nil {
